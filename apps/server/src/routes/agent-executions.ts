...
function sendError(
  ...
  if (
    error instanceof AgentExecutionServiceError
  ) {
    return reply
...
export async function agentExecutionRoutes(
  ...
  app.get(
    ...
    async (request) => {
      ...
      return getLiveProcessMetrics(id);
    },
  );

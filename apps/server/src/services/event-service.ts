...
function serialize(row: typeof domainEvents.$inferSelect): DomainEvent {
  return {
    ...
    agentExecutionId: row.agentExecutionId ?? null,
...
export async function recordEvent(
  input: Omit<typeof domainEvents.$inferInsert, "id" | "createdAt">,
): Promise<void> {
 ...
 */
export async function listRunEvents(runId: string): Promise<DomainEvent[]> {
  return (
    await db
      .select()
      .from(domainEvents)
 ...
 */
export async function listRecentEvents(limit = 8): Promise<DomainEvent[]> {

...
let originalResolutionTeamEnabled =
  true;
...
function makeProject(): Project {
  return {
    id:
      "project-document-worker-context-test",
    name:
      "orc-project-document-worker-context-test",
    path:
      `/tmp/orc-project-document-worker-context-${crypto.randomUUID()}`,
    branch:
      "main",
    gitState:
      "clean",
...
function createResult(
  ...
  return {
    status,
    summary:
      `Mock ${status} Project Document worker-context result.`,
    details: {},
    findings: [],
    filesChanged: [],
    commandsRun: [],
    ...
    await db
      ...
      .values({
        teamId:
          RESOLUTION_TEAM_ID,
        slug:
          `project-document-worker-context-${input.label
            .toLowerCase()
            .replaceAll(
              /[^a-z0-9]+/g,
              "-",
 ...
 */
function createBoundedDocumentContent():
  string {
  return Array
    .from(
      {
      ...
      (
        ...
        return [
          ...
          `worker context ${marker} ${"x".repeat(
            1_100,
          )}`,
        ].join(
          "\n",
 ...
 */
function queueCreateStartTurns():
  void {
 ...
 */
async function createConversationTaskRun(
  documentIds:
    string[],
) {
  const conversation =
 ...
 */
async function waitForWorkerLaunches(
  expectedCount:
    number,
): Promise<void> {
  for (
  ...
  throw new Error(
    `Timed out waiting for ${expectedCount} worker launches`,
  );
}
 ...
 */
async function loadRunDocumentContext(
  runId:
    string,
) {
  const [run] =
 ...
 */
async function mutateProjectDocument(
  documentId:
    string,
): Promise<void> {
...
describe(
  "Project Documents -> worker context regression",
  () => {
    /**
     ...
     */
    it(
      "carries attached Project Documents through Task creation and every forward worker from one immutable bounded Run snapshot",
      async () => {
        const first =
        ...
        const flow =
          await createConversationTaskRun(
            [
              uploaded
                .document
                .id,
     ...
     */
    it(
      "preserves the same snapshotted context through a configured changes_requested routing loop",
      async () => {
        const first =
          await createTestAgent({
            label:
     ...
     */
    it(
      "keeps unattached Project Documents out of Task, Run, worker, event, and monitoring context",
      async () => {
        const first =
          await createTestAgent({
            label:
              "Unattached First Worker",
            relativeLayer:
              1,
          });

...
type TerminalAction = "complete_run" | "fail_run" | "block_run";
...
type TransitionOrigin = "explicit" | "default" | "fallback" | "limit";
...
type WorkflowTransition = AgentWorkflowTransition | TerminalWorkflowTransition;
...
function snapshotFromRows(
  ...
  const orderedAgents = orderWorkflowAgents(agentRows);
  const enabledIds = new Set(orderedAgents.map((agent) => agent.id));
    ...
    routes: routeRows
      .filter(
        (route) =>
          ...
          (!route.targetAgentId || enabledIds.has(route.targetAgentId)),
      )
      .map((route) => ({
...
function snapshotOf(row: typeof runs.$inferSelect): WorkflowSnapshot {
  ...
  if (
    ...
    throw new WorkflowServiceError("Run workflow snapshot is invalid", 500);
  }
 ...
 */
function terminalStatusForAction(action: TerminalAction): TerminalRunStatus {
...
function resolveWorkflowTransition(
  ...
  const explicitRoute = snapshot.routes.find(
    (route) =>
      route.sourceAgentId === sourceAgentId && route.outcome === outcome,
  );
  ...
  if (explicitRoute?.terminalAction) {
    return {
      ...
      outcome,
      targetAgentId: null,
      terminalAction: explicitRoute.terminalAction,
      reason: `Terminal route: ${explicitRoute.terminalAction}`,
    };
  }

  if (explicitRoute?.targetAgentId) {
    const targetExists = snapshot.agents.some(
      (agent) => agent.id === explicitRoute.targetAgentId,
    );
    ...
    if (!targetExists) {
      return {
        ...
        attemptedTargetAgentId: explicitRoute.targetAgentId,
      };
    }
    ...
    return {
      ...
      sourceAgentId,
      outcome,
      targetAgentId: explicitRoute.targetAgentId,
      terminalAction: null,
    };
  }
  ...
  if (outcome === "completed" || outcome === "approved") {
    const nextAgent = snapshot.agents[sourceIndex + 1];
  ...
  const terminalAction: TerminalAction =
    outcome === "blocked" || outcome === "changes_requested"
      ? "block_run"
      : "fail_run";
  ...
  return {
    ...
    targetAgentId: null,
    terminalAction,
    reason: failureReason ?? `No configured route for ${outcome}.`,
  };
}
...
function enforceWorkflowExecutionLimit(
  ...
  if (
    ...
    executionCount < env.MAX_WORKFLOW_EXECUTIONS
  ) {
    return transition;
  }
  ...
  return {
    kind: "terminal",
    origin: "limit",
    sourceAgentId: transition.sourceAgentId,
    outcome: transition.outcome,
    targetAgentId: null,
    terminalAction: "fail_run",
    reason: `Workflow execution limit (${env.MAX_WORKFLOW_EXECUTIONS}) reached.`,
    attemptedTargetAgentId: transition.targetAgentId,
  };
}
...
function transitionEventData(
  ...
  const data: Record<string, unknown> = {
    ...
    sourceAgentId: transition.sourceAgentId,
    outcome: transition.outcome,
    targetAgentId: transition.targetAgentId,
    terminalAction: transition.terminalAction,
    executionCount,
  };
  ...
  if (transition.kind === "terminal") {
    ...
    if (transition.attemptedTargetAgentId) {
      data.attemptedTargetAgentId = transition.attemptedTargetAgentId;
    }
  }
  ...
  const transitioned = await db.transaction(async (tx) => {
    const condition = expectedAgentId
      ? and(
          ...
          eq(runs.currentAgentId, expectedAgentId),
        )
      : and(eq(runs.id, run.id), inArray(runs.status, ["pending", "running"]));
...
async function getTaskInstruction(
  ...
  if (!run.taskId) {
    throw new WorkflowServiceError("Workflow run is missing its task", 500);
  }
  ...
  if (!task) {
    throw new WorkflowServiceError("Workflow task no longer exists", 500);
  }
...
function resolveExecutionAgent(
  ...
  if (!override?.harness && !override?.model && !override?.reasoning) {
...
async function launchClaimedAgent(
  ...
  const agent = resolveExecutionAgent(snapshotAgent, override);

  try {
    const baseInstruction = await getTaskInstruction(claimedRun);
    ...
    await recordEvent({
      type: "agent.started",
      projectPath: claimedRun.projectPath,
      ...
      data: {
        ...
        model: agent.model,
        reasoning: agent.reasoning,
        executionCount: claimedRun.executionCount,
        ...(override?.harness || override?.model || override?.reasoning
          ? {
              overridden: true,
            }
          : {}),
    ...
    await startSnapshotAgentExecution(
      ...
      instruction,
      (finalization) =>
        handleExecutionFinalization(claimedRun.id, agent.id, finalization),
    );
  } catch (error) {
...
async function claimAndLaunchAgent(
  ...
  if (!run || run.status !== "running" || run.currentAgentId !== null) {
    return;
  }
  ...
  try {
    ...
    await updateTerminal(run, "failed", errorMessage(error));
    return;
  }
  ...
  if (run.executionCount >= env.MAX_WORKFLOW_EXECUTIONS) {
    await updateTerminal(
      run,
      "failed",
  ...
  const [claimed] = await db
    .update(runs)
    .set({
      currentAgentId: snapshotAgent.id,
      executionCount: run.executionCount + 1,
      updatedAt: new Date(),
    })
    .where(
      and(
  ...
  await launchClaimedAgent(claimed, snapshotAgent, handoffNote, override);
}
...
async function applyFinalizationTransition(
  ...
  return db.transaction(async (tx) => {
    ...
    if (!run || run.status !== "running" || run.currentAgentId !== agentId) {
      return null;
    }
    ...
    const transition = enforceWorkflowExecutionLimit(
      resolveWorkflowTransition(snapshot, agentId, resultStatus, failureReason),
      run.executionCount,
    );
    ...
    const activeSourceCondition = and(
      eq(runs.id, run.id),
      eq(runs.status, "running"),
      eq(runs.currentAgentId, agentId),
    );

    let updatedRun: typeof runs.$inferSelect | undefined;

    let targetAgent: SnapshotAgent | null = null;
    ...
    if (transition.kind === "agent") {
      targetAgent =
        snapshot.agents.find(
          (candidate) => candidate.id === transition.targetAgentId,
        ) ?? null;
      ...
      terminalStatus = terminalStatusForAction(transition.terminalAction);
    ...
    const events: Array<typeof domainEvents.$inferInsert> = [
      {
      ...
      },
      {
        ...
        runId: run.id,
        agentExecutionId: executionId,
        data: transitionEventData(transition, run.executionCount),
      },
    ];

    if (transition.origin === "explicit" && transition.kind === "agent") {
      events.push({
        type: "route.selected",
        projectPath: run.projectPath,
        taskId: run.taskId,
    ...
    if (terminalStatus) {
      events.push({
        ...
        runId: run.id,
        data: {
          reason: transition.kind === "terminal" ? transition.reason : null,
        },
      });
    }

    await tx.insert(domainEvents).values(events);
...
async function handleExecutionFinalization(
  ...
  if (finalization.status === "cancelled") {
    await updateTerminal(run, "cancelled", finalization.failureReason, agentId);
    return;
  }
  ...
  let applied: AppliedWorkflowTransition | null;

  try {
    applied = await applyFinalizationTransition(
      runId,
      agentId,
  ...
  if (applied.transition.kind === "terminal") {
    requestAutoModeCycle();
    return;
  }
  ...
  if (!applied.targetAgent) {
    await updateTerminal(
      ...
      applied.run.currentAgentId ?? undefined,
    );
    return;
  }

  const handoffNote =
    finalization.result && applied.sourceAgent
      ? composeHandoffNote(applied.sourceAgent, finalization.result)
      : undefined;

  await launchClaimedAgent(applied.run, applied.targetAgent, handoffNote);
}
 ...
 */
export async function startTask(id: string): Promise<TaskWithRun | null> {
  ...
  if (existingTask.status !== "pending") {
    throw new WorkflowServiceError("Only a pending task can be started", 409);
  }
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (active.length) {
      throw new WorkflowServiceError("Another task is already active", 409);
    }

    const [currentTask] = await tx.select().from(tasks).where(eq(tasks.id, id));

    if (!currentTask) {
      throw new WorkflowServiceError("The task no longer exists", 404);
    }
    ...
    const routes = await tx
      .select()
      .from(agentRoutes)
      .where(eq(agentRoutes.enabled, true));

    const workflowSnapshot = snapshotFromRows(enabledAgents, routes);
  ...
  void claimAndLaunchAgent(result.run.id, snapshot.agents[0].id);
...
export async function listTasks(): Promise<Task[]> {
  return (await db.select().from(tasks).orderBy(desc(tasks.createdAt))).map(
    serializeTask,
  );
}
...
export async function listRuns(): Promise<Run[]> {
  return (await db.select().from(runs).orderBy(desc(runs.createdAt))).map(
    serializeRun,
  );
}
...
export async function getRunDetail(id: string): Promise<{
  ...
  const executions = (
    await Promise.all(rows.map((row) => getExecution(row.id)))
  ).filter((value): value is AgentExecution => value !== null);

  const [task] = run.taskId
    ? await db.select().from(tasks).where(eq(tasks.id, run.taskId))
    : [];
...
export async function cancelRun(id: string): Promise<Run | null> {
  ...
  if (run.status !== "running" && run.status !== "pending") {
    throw new WorkflowServiceError("Only an active run can be cancelled", 409);
  }
  ...
  if (execution) {
    await cancelLiveExecution(execution.id);
...
export async function retryLastExecution(
  ...
  if (!snapshot.agents.some((agent) => agent.id === execution.agentId)) {
    throw new WorkflowServiceError(
  ...
  const updated = await db.transaction(async (tx) => {
    ...
    const [retriedRun] = await tx
      ...
      .where(
        and(
          eq(runs.id, id),
          inArray(runs.status, ["failed", "blocked"]),
          sql`${runs.currentAgentId} is null`,
        ),
      )
      .returning();
    ...
    if (retriedRun.taskId) {
      await tx
        ...
        .where(eq(tasks.id, retriedRun.taskId));
    }

    await tx.insert(domainEvents).values({
      type: "execution.retried",
      projectPath: retriedRun.projectPath,
  ...
  void claimAndLaunchAgent(id, execution.agentId, undefined, override);
...
export async function recoverInterruptedWorkflows(): Promise<void> {
  ...
  for (const run of active) {
    const transitioned = await updateTerminal(
      run,
      "blocked",
    ...
    await db
      ...
      .set({
        status: "blocked",
        failureReason: "Server restarted while worker state was unavailable.",
        completedAt: new Date(),
        updatedAt: new Date(),
      })
      .where(
        and(
          ...
          inArray(agentExecutions.status, ["pending", "starting", "running"]),
        ),
      );
  }
}

...
// assistant message text from harness-specific stream-json fields. Non-JSON lines (used by the
// pre-existing chunk-ordering tests below) are ignored by translateOutput, same as production
// adapters do for malformed lines.
function makeFakeAdapter(harness: StartWorkerInput["agent"]["harness"]): HarnessAdapter {
  return {
    harness,
    ...
    extractMessageText: (event) => (typeof event.text === "string" ? event.text : undefined),
  };
}

// The service resolves its own HarnessAdapter via getHarnessAdapter(agent.harness) (matching how
// runtime/session.ts resolves the adapter it uses for translateOutput) rather than being handed
// one by startWorker, so this must be mocked too and return the SAME fake adapter shape used by
// the mocked startWorker() below -- otherwise the service would decode provider events using the
// real claude/codex field names instead of this test's `{"text": "..."}` convention.
...
// flow (which triggers a second startWorker() call reusing the same execution row) can drive
// both attempts independently.
let ptyInstances: FakePty[] = [];

function pty(index: number): FakePty {
  const instance = ptyInstances[index];
  if (!instance) throw new Error(`Expected a spawned PTY at index ${index}`);
  return instance;
}

function feedResult(instance: FakePty, payload: Record<string, unknown>): void {
  instance.data(JSON.stringify({ text: `<orc-result>${JSON.stringify(payload)}</orc-result>` }));
}
...
vi.mock("../runtime/index.js", async (importOriginal) => {
  ...
  return {
    ...
    startWorker: (input: StartWorkerInput) => {
      ...
      const factory: PtyFactory = { spawn: () => instance };
...
const { createRun, startAgentExecution, getExecution, listTerminalChunks } = await import("./agent-execution-service.js");

async function waitFor<T>(check: () => Promise<T | undefined | false>, timeoutMs = 2000): Promise<T> {
...
describe("agent-execution-service", () => {
  ...
  beforeEach(async () => {
    projectPath = fs.mkdtempSync(path.join(os.tmpdir(), "orc-execution-service-"));
    const [agent] = await db
      ...
      .values({
        ...
        name: "Test Agent",
        role: "Tester",
        layer: (agentLayer = 900 + Math.floor(Math.random() * 100_000)),
        executionOrder: 1,
        harness: "codex",
        model: "gpt-5",
        reasoning: "high",
  ...
  it("persists denormalized agent fields when starting an execution", async () => {
    const execution = await startAgentExecution(runId, agentId, "Inspect the repository.");
    ...
    expect(persisted).toMatchObject({ agentName: "Test Agent", harness: "codex" });
  });
  ...
  it("produces ordered terminal_chunks rows from output events and records the pid once running", async () => {
    ...
    expect(chunks.map((chunk) => chunk.data)).toEqual(["one", "two"]);
  ...
  it("finalizes on a valid first-attempt result without triggering a repair", async () => {
    ...
    const payload = { status: "completed", summary: "Implemented the requested change.", commit: null };
  ...
  it("repairs a malformed first attempt and finalizes on a valid repair result", async () => {
    ...
    pty(0).data(JSON.stringify({ text: "No result block here." }));
    pty(0).exit(0);
    ...
    const repairPayload = { status: "changes_requested", summary: "Found issues to address.", commit: null };
    ...
    const finalized = await waitFor(async () => {
      const current = await getExecution(execution.id);
      return current?.resultStatus ? current : undefined;
    });
  ...
  it("fails the execution when both the first attempt and the repair attempt are invalid", async () => {
    ...
    pty(0).data(JSON.stringify({ text: "No result block here either." }));
    pty(0).exit(1);
    ...
    pty(1).data(JSON.stringify({ text: "Still no result block." }));
    pty(1).exit(1);
  ...
  it("treats a reported commit from an agent without commit permission as invalid and triggers a repair", async () => {
    ...
    feedResult(pty(0), { status: "completed", summary: "Implemented the change.", commit: "abc1234" });
    pty(0).exit(0);
    ...
    const correctedPayload = { status: "completed", summary: "Implemented the change.", commit: null };

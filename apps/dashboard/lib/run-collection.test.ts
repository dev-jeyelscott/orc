...
import type { RunMonitoringSummary } from "@orc/shared";
...
function testFiltering(): void {
  const shop = createRun();
  const billing = createRun({
  ...
  assert.deepEqual(
    filterRunCollection([shop, billing], {
      search: "payment",
      status: "failed",
      projectPath: "/orc/workspace/billing-api",
    }).map((run) => run.id),
...
function testSorting(): void {
  ...
  assert.deepEqual(
    sortRunCollection([older, newer], "newest").map((run) => run.id),
    [newer.id, older.id],
  );

  assert.deepEqual(
    sortRunCollection([older, newer], "oldest").map((run) => run.id),
    [older.id, newer.id],
  );
}
...
function testProjectOptions(): void {
  ...
  assert.equal(getRunProjectName("/orc/workspace/shop-portal/"), "shop-portal");

  assert.deepEqual(getRunProjectOptions(runs), [
    {
      value: "/orc/workspace/billing-api",
      label: "billing-api",
    },
    {
      value: "/orc/workspace/shop-portal",
      label: "shop-portal",
    },
  ]);
}
...
function testViewIndependentCollection(): void {
  ...
  const result = deriveRunCollection(
    [older, newer],
    {
      search: "",
      status: "all",
...
console.log("run collection helper tests passed");

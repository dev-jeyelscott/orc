...
import type { AgentExecution, RunMonitoringDetail } from "@orc/shared";
...
function testPreferredExecution(): void {
  ...
  assert.equal(selectPreferredExecutionId([completed, running]), running.id);

  const latest = createExecution({
    createdAt: "2026-09-04T00:20:00.000Z",
  });

  assert.equal(selectPreferredExecutionId([completed, latest]), latest.id);
}
...
function testCurrentState(): void {
  ...
  assert.equal(currentRunStateLabel(createDetail([running])), "Executing");
}
...
function testProgress(): void {
  assert.equal(executionProgress(3, 6), 50);

  assert.equal(executionProgress(8, 6), 100);

  assert.equal(executionProgress(3, 0), 0);
}
...
function testRunStatePredicates(): void {
  ...
  assert.equal(isRunRetryable("blocked"), true);

  assert.equal(runStatusVariant("failed"), "error");
}
...
function testTerminalDuration(): void {
  ...
  assert.equal(terminalRunDurationMs(detail.run), null);
...
console.log("run-detail-state helper tests passed");

...
function createRun(
  ...
  return {
    ...
    teamId: TEAM_ID,
    projectPath: "/orc/workspace/shop-portal",
    status: "completed",
    currentAgentId: null,
    ...
    taskTitle: "Implement checkout retry flow",
...
function testFiltering(): void {
  ...
  const failed = createRun({
    ...
    projectPath: "/orc/workspace/billing-api",
    status: "failed",
  });

  assert.deepEqual(
    filterRunSummaries([completed, failed], "payment", "all").map(
      (run) => run.id,
    ),
    [failed.id],
  );
  ...
  assert.deepEqual(
    filterRunSummaries([completed, failed], "", "completed").map(
      (run) => run.id,
    ),
    [completed.id],
  );
...
function testMetrics(): void {
  const runs = [
    ...
    createRun({
      status: "cancelled",
      updatedAt: "2026-09-04T00:40:00.000Z",
    }),
  ];

  const metrics = calculateRunMetrics(runs);

  assert.equal(Math.round(metrics.successRate ?? 0), 33);

  assert.equal(metrics.failedBlocked, 2);

  assert.equal(metrics.medianDurationMs, 25 * 60 * 1000);
}
...
function testUsage(): void {
  ...
  const aggregate = aggregateRunUsage([
    createExecution(agent, {
      tokenUsage: {
        input_tokens: 100,
        output_tokens: 50,
      },
  ...
  assert.equal(aggregate.tokens?.totalTokens, 150);

  assert.equal(aggregate.context?.percent, 50);
}
...
function testWorkflowPlan(): void {
  const first = createPlanAgent({
    name: "Worker A",
    layer: 1,
    executionOrder: 1,
  });

  const second = createPlanAgent({
    name: "Worker B",
    layer: 2,
    executionOrder: 1,
  });
  ...
  const execution = createExecution(first, {
    runId,
  });
  ...
  const detail: RunMonitoringDetail = {
    ...
    events: [],
    executionPlan: [first, second],
  };

  const steps = deriveWorkflowSteps(
    detail,
    Date.parse("2026-09-04T00:10:00.000Z"),
  );
...
function testChartBuckets(): void {
  ...
  const buckets = buildRunsOverTime(
    [
      createRun({
...
console.log("run-observability helper tests passed");

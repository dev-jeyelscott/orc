...
export function RunDetail({ runId }: { runId: string }) {
  ...
  async function retry() { setCancelling(true); try { await retryRun(runId); load(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to retry run"); } finally { setCancelling(false); } }
  ...
  return <div className="flex flex-col gap-6">
    ...
    {(run.status === "failed" || run.status === "blocked") && <Button onClick={() => void retry()} disabled={cancelling}>Retry final execution</Button>}

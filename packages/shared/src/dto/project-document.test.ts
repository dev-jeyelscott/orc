...
describe("project document contracts", () => {
  /**
   * Verifies canonical Markdown and plain-text upload metadata are accepted.
   */
  it("accepts supported bounded upload metadata", () => {
    expect(
   ...
   */
  it("rejects unsupported or unsafe upload metadata", () => {
    expect(
      projectDocumentUploadMetadataSchema.safeParse({
    ...
    expect(
      projectDocumentUploadMetadataSchema.safeParse({
        ...
        extension: ".md",
        mediaType: "text/markdown",
        sizeBytes: MAX_PROJECT_DOCUMENT_UPLOAD_BYTES + 1,
      }).success,
    ).toBe(false);
  });
   ...
   */
  it("accepts persisted project document metadata", () => {
    const parsed = projectDocumentMetadataSchema.parse({
      id: DOCUMENT_ID,
      teamId: TEAM_ID,
    ...
    expect(parsed.contentHash).toBe(DOCUMENT_HASH);
   ...
   */
  it("validates deterministic chunk metadata", () => {
    expect(
   ...
   */
  it("enforces uploaded document context item bounds", () => {
    expect(
      uploadedProjectDocumentContextRefSchema.safeParse({
        ...
        excerpt: "x".repeat(MAX_PROJECT_DOCUMENT_EXCERPT_CHARS),
      }).success,
    ).toBe(true);
    ...
    expect(
      uploadedProjectDocumentContextRefSchema.safeParse({
        ...
        chunkSequence: 0,
        chunkContentHash: CHUNK_HASH,
        excerpt: "x".repeat(MAX_PROJECT_DOCUMENT_EXCERPT_CHARS + 1),
      }).success,
    ).toBe(false);
  });
   ...
   */
  it("enforces uploaded document context collection bounds", () => {
    const withinLimit = Array.from(
      {
        length: MAX_PROJECT_DOCUMENT_CONTEXT_ITEMS,
      },
      (_, index) => ({
        ...
        documentContentHash: DOCUMENT_HASH,
        chunkSequence: index,
        chunkContentHash: index.toString(16).padStart(64, "0"),
        excerpt: "x",
      }),
    );

    expect(
      uploadedProjectDocumentContextCollectionSchema.safeParse(withinLimit)
        .success,
    ).toBe(true);
    ...
    expect(
      uploadedProjectDocumentContextCollectionSchema.safeParse([
        ...withinLimit,
        {
          ...
          fileName: "roadmap.md",
          documentContentHash: DOCUMENT_HASH,
          chunkSequence: MAX_PROJECT_DOCUMENT_CONTEXT_ITEMS,
          chunkContentHash: "f".repeat(64),
          excerpt: "x",
        },
      ]).success,
    ).toBe(false);
   ...
   */
  it("keeps uploaded document and vault provenance distinct", () => {
    expect(
      knowledgeRefSchema.safeParse({
        source: "vault",
        path: "Projects/orc/README.md",
        excerpt: "Vault context.",
      }).success,
    ).toBe(true);
   ...
   */
  it("validates create requests against file metadata agreement", () => {
    expect(
      createProjectDocumentRequestSchema.safeParse({
        teamId: TEAM_ID,
        projectPath: "/tmp/orc-test-project",
        fileName: "roadmap.md",
        extension: ".md",
        mediaType: "text/markdown",
        content: "# Roadmap",
   ...
   */
  it("keeps list and create responses free of chunk content", () => {
    const document = {
      id: DOCUMENT_ID,
      teamId: TEAM_ID,
      projectPath: "/tmp/orc-test-project",
    ...
    const listParsed = projectDocumentListResponseSchema.safeParse({
      documents: [document],
    });

    expect(listParsed.success).toBe(true);

    const createParsed = projectDocumentCreateResponseSchema.safeParse({
      document,
      chunkCount: 3,
    });

...
describe("run contracts", () => {
  it("requires persisted Runs to expose Team ownership", () => {
    expect(runSchema.parse(runPayload()).teamId).toBe(TEAM_ID);
  });

  it("rejects a persisted Run without Team ownership", () => {
    const { teamId: _teamId, ...withoutTeam } = runPayload();

    expect(runSchema.safeParse(withoutTeam).success).toBe(false);
  });
});

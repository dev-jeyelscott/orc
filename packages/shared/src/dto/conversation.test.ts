...
import { MAX_PROJECT_DOCUMENT_SELECTIONS } from "./project-document.js";
...
const teamId = "00000000-0000-4000-9000-000000000001";
...
describe("conversation contracts", () => {
   ...
   */
  it("accepts persisted Team-scoped Conversation detail", () => {
    ...
    expect(result.conversation.teamId).toBe(teamId);
   ...
   */
  it("requires Project and Team when creating a Conversation", () => {
    expect(
   ...
   */
  it("preserves legacy content-only message parsing", () => {
    expect(
      postConversationMessageSchema.parse({
        content: "Use the current project state.",
      }),
    ).toEqual({
      content: "Use the current project state.",
    });
   ...
   */
  it("accepts bounded document IDs on Conversation posts", () => {
    expect(
      postConversationMessageSchema.parse({
        content: "Use the selected roadmap.",
   ...
   */
  it("rejects invalid document selections", () => {
    expect(
    ...
    expect(
      postConversationMessageSchema.safeParse({
        ...
        documentIds: Array.from(
          {
            length: MAX_PROJECT_DOCUMENT_SELECTIONS + 1,
          },
          (_, index) =>
   ...
   */
  it("keeps Conversation message posts strict", () => {
    expect(
      postConversationMessageSchema.safeParse({
        content: "Hello",
        unexpected: true,
   ...
   */
  it("rejects model-selected Team input on create_task", () => {
    expect(

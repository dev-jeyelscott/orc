...
import { projectSchema, upsertProjectTeamAssignmentSchema } from "./project.js";
...
describe("Project assignment DTO contracts", () => {
  ...
  it("bounds assignment fields before server-side automation validation", () => {
    expect(
      upsertProjectTeamAssignmentSchema.parse({
        teamId: TEAM_ID,
    ...
    expect(
      upsertProjectTeamAssignmentSchema.safeParse({
        teamId: TEAM_ID,
